<?php
	class ModelSimpleBlogArticle extends Model {
		...
		public function getArticleLimit($limit, $start) {
			//echo "SELECT sba . * , sbad . * , sbau.name AS author_name FROM `sm_simple_blog_article` sba LEFT JOIN `sm_simple_blog_article_description` sbad ON ( sba.simple_blog_article_id = sbad.simple_blog_article_id ) LEFT JOIN `sm_simple_blog_article_to_store` sbas ON ( sba.simple_blog_article_id = sbas.simple_blog_article_id ) LEFT JOIN `sm_simple_blog_author` sbau ON ( sba.simple_blog_author_id = sbau.simple_blog_author_id ) WHERE sba.status = 1 AND sbad.language_id = 1 ORDER BY sba.date_modified DESC LIMIT $start , $limit"; die();
			$sql = "SELECT sba . * , sbad . * , sbau.name AS author_name FROM `sm_simple_blog_article` sba LEFT JOIN `sm_simple_blog_article_description` sbad ON ( sba.simple_blog_article_id = sbad.simple_blog_article_id ) LEFT JOIN `sm_simple_blog_article_to_store` sbas ON ( sba.simple_blog_article_id = sbas.simple_blog_article_id ) LEFT JOIN `sm_simple_blog_author` sbau ON ( sba.simple_blog_author_id = sbau.simple_blog_author_id ) WHERE sba.status = 1 AND sbad.language_id = 1 ORDER BY sba.date_modified DESC LIMIT $start , $limit";
			$query = $this->db->query($sql);			
			return $query->rows;
		}
		public function getArticlesBlogs($id_) {
			$sql = "SELECT sba . * , sbad . * , sbau.name AS author_name FROM `sm_simple_blog_article` sba LEFT JOIN `sm_simple_blog_article_description` sbad ON ( sba.simple_blog_article_id = sbad.simple_blog_article_id ) LEFT JOIN `sm_simple_blog_article_to_store` sbas ON ( sba.simple_blog_article_id = sbas.simple_blog_article_id ) LEFT JOIN `sm_simple_blog_author` sbau ON ( sba.simple_blog_author_id = sbau.simple_blog_author_id ) WHERE sba.simple_blog_article_id = ".$id_." AND sbad.language_id = 1";					
			$query = $this->db->query($sql);	
			return $query->rows;
		}
